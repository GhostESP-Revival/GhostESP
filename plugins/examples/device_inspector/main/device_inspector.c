...
typedef enum {
    PAGE_MENU, PAGE_SYSTEM, PAGE_WIFI, PAGE_BLE, PAGE_RGB,
    PAGE_STORAGE, PAGE_STORAGE_TEST, PAGE_GPS, PAGE_HARDWARE,
    PAGE_CANVAS, PAGE_INPUT, PAGE_THEME, PAGE_UNSAFE,
} page_id_t;
...
typedef struct { const char *label; page_id_t page; } menu_page_t;
static const menu_page_t s_menu_pages[] = {
    { "System Info", PAGE_SYSTEM },   { "WiFi Scan", PAGE_WIFI },
    { "BLE Scan", PAGE_BLE },         { "RGB Test", PAGE_RGB },
    { "Storage Browser", PAGE_STORAGE }, { "Storage R/W Test", PAGE_STORAGE_TEST },
    { "GPS Status", PAGE_GPS },       { "Hardware Info", PAGE_HARDWARE },
    { "Canvas Demo", PAGE_CANVAS },   { "Input Tester", PAGE_INPUT },
    { "Theme Colors", PAGE_THEME },   { "Unsafe Probe", PAGE_UNSAFE },
};

static void navigate_up(void);
static void navigate_down(void);
static void navigate_back(void);
static void touch_up_clicked(void *user) { (void)user; navigate_up(); }
static void touch_down_clicked(void *user) { (void)user; navigate_down(); }
static void touch_back_clicked(void *user) { (void)user; navigate_back(); }
...
static void detail_back(void *user) {
    (void)user;
    if (detail_view && api->ui_detail_destroy) { api->ui_detail_destroy(detail_view); detail_view = NULL; }
    show_menu();
}
...
static void destroy_subviews(void) {
    destroy_touch_controls();
    if (detail_view && api->ui_detail_destroy) { api->ui_detail_destroy(detail_view); detail_view = NULL; }
    if (storage_menu && api->ui_options_destroy) { api->ui_options_destroy(storage_menu); storage_menu = NULL; }
    if (ble_menu && api->ui_options_destroy) { api->ui_options_destroy(ble_menu); ble_menu = NULL; }
}

static void stop_canvas_timer(void) {
    if (canvas_timer && api->ui_timer_delete) { api->ui_timer_delete(canvas_timer); canvas_timer = NULL; }
    canvas_obj = NULL;
}
...
static void popup_close(void *user) {
    ...
    if (popup && api->ui_popup_destroy) { api->ui_popup_destroy(popup); popup = NULL; }
}

static void exit_app(void *user) { (void)user; GH_VOID(api, app_exit); }

static void menu_select(void *user) {
    int idx = (int)(intptr_t)user;
    if (idx >= 0 && idx < ARRAY_COUNT(s_menu_pages)) {
        if (main_menu && api->ui_options_destroy) { api->ui_options_destroy(main_menu); main_menu = NULL; }
...
static void open_system(void) {
    ...
    gh_detail_printf(detail_view, api, "Uptime", "%lu ms", (unsigned long)api->system_uptime_ms());
    gh_detail_printf(detail_view, api, "Free Heap", "%lu bytes", (unsigned long)api->system_free_heap());
    gh_detail_printf(detail_view, api, "Free Internal", "%lu bytes", (unsigned long)api->system_free_internal_heap());

    size_t used = api->app_memory_used ? api->app_memory_used() : 0;
    size_t limit = api->app_memory_limit ? api->app_memory_limit() : 0;
    gh_detail_printf(detail_view, api, "App Memory", "%lu / %lu", (unsigned long)used, (unsigned long)limit);

    api->ui_detail_add_info(detail_view, "App ID", api->app_id ? api->app_id() : "unknown");
    api->ui_detail_add_info(detail_view, "Data Path", api->app_data_path ? api->app_data_path() : "unknown");
    gh_detail_printf(detail_view, api, "Screen", "%ld x %ld",
        (long)(api->ui_screen_get_width ? api->ui_screen_get_width() : 0),
        (long)(api->ui_screen_get_height ? api->ui_screen_get_height() : 0));
    gh_detail_printf(detail_view, api, "Flags", "0x%08lX", (unsigned long)api->flags);
    ...
    gh_detail_printf(detail_view, api, "Theme", "%u", (unsigned)(api->settings_get_theme ? api->settings_get_theme() : 0));
...
static void wifi_scan_done(void *user) {
    if (scan_status && api->ui_scan_status_close) { api->ui_scan_status_close(scan_status); scan_status = NULL; }
    if (!api->ui_detail_create) return;
    detail_view = api->ui_detail_create("WiFi Scan Results");
    if (!detail_view) return;

    uint16_t count = api->wifi_ap_count ? api->wifi_ap_count() : 0;
    gh_detail_printf(detail_view, api, "Summary", "%u APs found", (unsigned)count);
    ...
    if ((int)count > show) gh_detail_printf(detail_view, api, "", "... and %u more", (unsigned)(count - show));
...
static void ble_track_selected(void *user) {
    ...
    if (ble_detail_index >= 0 && api->ble_detect_start_tracking) {
        if (!api->ble_detect_start_tracking(ble_detail_index))
            popup = gh_popup(api, 260, 180, "BLE Detect", "Unable to start tracking", "OK", popup_close, NULL);
    }
}
...
static void ble_spoof_selected(void *user) {
    ...
    if (ble_detail_index >= 0 && api->ble_detect_start_airtag_spoof) {
        if (!api->ble_detect_start_airtag_spoof(ble_detail_index))
            popup = gh_popup(api, 260, 180, "BLE Detect", "Unable to start AirTag spoof", "OK", popup_close, NULL);
    }
}
...
static void ble_select(void *user) {
    ...
    gh_detail_printf(detail_view, api, "RSSI", "%d dBm", dev->rssi);
...
static void ble_back(void *user) { (void)user; show_menu(); }
...
static void ble_activate_selected(void) {
    ...
    if (selected >= 0 && selected < ble_count) ble_select((void *)(intptr_t)selected);
    else show_menu();
}
...
static void rgb_timer_cb(void *user) {
    ...
    uint8_t r = 0, g = 0, b = 0;
    switch (rgb_step % 8) {
        case 0: r = 255; break; case 1: g = 255; break; case 2: b = 255; break;
        case 3: r = 255; g = 255; break; case 4: g = 255; b = 255; break;
        case 5: r = 255; b = 255; break; case 6: r = 255; g = 255; b = 255; break;
        case 7: break;
    }
    api->rgb_set_all(r, g, b);
    char msg[64];
    snprintf(msg, sizeof(msg), "Step %d: RGB(%d,%d,%d)", rgb_step % 8, r, g, b);
    GH_VOID(api, ui_set_status, msg);
}

static void rgb_stop(void *user) {
    if (rgb_timer && api->ui_timer_delete) { api->ui_timer_delete(rgb_timer); rgb_timer = NULL; }
...
static void open_rgb(void) {
    ...
    gh_detail_section(detail_view, api, "Cycles through 8 colors automatically");
    ...
    for (int i = 0; i < 8; i++)
        gh_detail_printf(detail_view, api, "", "%s (#%06lX)", names[i], (unsigned long)colors[i]);

    api->ui_detail_add_action(detail_view, "Stop Test", rgb_stop, NULL);
    api->ui_detail_add_back(detail_view, detail_back, NULL);
    create_touch_controls(true);
    if (api->ui_timer_create) rgb_timer = api->ui_timer_create(rgb_timer_cb, 500, NULL);
}

static bool storage_is_root(void) { return strcmp(storage_path, "/mnt/ghostesp") == 0; }

static void storage_go_parent(void) {
    if (storage_is_root()) { show_menu(); return; }
    char *slash = strrchr(storage_path, '/');
    if (!slash || slash <= storage_path + strlen("/mnt/ghostesp")) strcpy(storage_path, "/mnt/ghostesp");
    else *slash = '\0';
    open_storage();
}

static void storage_back(void *user) { (void)user; storage_go_parent(); }
...
static void storage_select(void *user) {
    ...
    if (encoded == 0) { storage_go_parent(); return; }
    int idx = encoded - 1;
    if (idx < 0 || idx >= storage_count) return;
...
static void storage_activate_selected(void) {
    ...
    if (!storage_is_root() && selected == 0) { storage_go_parent(); }
...
static void open_storage_test(void) {
    ...
    gh_detail_section(detail_view, api, "Write / Read / Delete cycle");
    ...
    if (n > 0) api->ui_detail_add_info(detail_view, "   Content", readback);
...
static void open_gps(void) {
    ...
    if (avail) {
        api->ui_detail_add_info(detail_view, "Fix", (api->gps_has_fix && api->gps_has_fix()) ? "YES" : "NO");
        gh_detail_printf(detail_view, api, "Latitude", "%.6f", api->gps_get_latitude ? api->gps_get_latitude() : 0.0);
        gh_detail_printf(detail_view, api, "Longitude", "%.6f", api->gps_get_longitude ? api->gps_get_longitude() : 0.0);
        gh_detail_printf(detail_view, api, "Altitude", "%.1f m", api->gps_get_altitude ? api->gps_get_altitude() : 0.0);
        gh_detail_printf(detail_view, api, "Satellites", "%d", api->gps_get_satellites ? api->gps_get_satellites() : 0);
        gh_detail_printf(detail_view, api, "Speed", "%.1f km/h", api->gps_get_speed ? api->gps_get_speed() : 0.0f);
        gh_detail_printf(detail_view, api, "Heading", "%.1f deg", api->gps_get_heading ? api->gps_get_heading() : 0.0f);
    } else {
...
static void open_hardware(void) {
    ...
    gh_detail_section(detail_view, api, "Radio Hardware");
    ...
    gh_detail_section(detail_view, api, "Display");
    gh_detail_printf(detail_view, api, "Resolution", "%ld x %ld px",
        (long)(api->ui_screen_get_width ? api->ui_screen_get_width() : 0),
        (long)(api->ui_screen_get_height ? api->ui_screen_get_height() : 0));
    if (api->ui_theme_is_bright)
        api->ui_detail_add_info(detail_view, "Theme Mode", api->ui_theme_is_bright() ? "Bright" : "Dark");

    gh_detail_section(detail_view, api, "Memory");
    gh_detail_printf(detail_view, api, "Heap", "%lu bytes free", (unsigned long)(api->system_free_heap ? api->system_free_heap() : 0));
    gh_detail_printf(detail_view, api, "Internal", "%lu bytes free", (unsigned long)(api->system_free_internal_heap ? api->system_free_internal_heap() : 0));
...
static void canvas_tick_cb(void *user) {
    ...
    api->ui_canvas_fill(canvas_obj, theme.bg);
    ...
    if (api->ui_canvas_draw_arc)
        api->ui_canvas_draw_arc(canvas_obj, cx, cy, radius > 5 ? radius : 5, 0, 360, theme.accent, 2);

    int bars = 8;
    int bar_w = (w - 20) / bars;
    for (int i = 0; i < bars; i++) {
        int bh = ((canvas_tick * 3 + i * 37) % (h - 20)) + 5;
        api->ui_canvas_draw_rect(canvas_obj, 10 + i * bar_w, h - bh - 5, bar_w - 2, bh, (i % 2 == 0) ? theme.accent : theme.surface);
    }

    static const int8_t orbit_x[6] = {100, 50, -50, -100, -50, 50};
    static const int8_t orbit_y[6] = {0, 87, 87, 0, -87, -87};
    int orbit = max_r * 7 / 10;
    int phase = (canvas_tick / 4) % 6;
    for (int i = 0; i < 6; i++) {
        int p = (i + phase) % 6;
        int px = cx + orbit * orbit_x[p] / 100;
        int py = cy + orbit * orbit_y[p] / 100;
        api->ui_canvas_draw_rect(canvas_obj, px - 2, py - 2, 5, 5, theme.text);
    }
}

static void canvas_stop(void *user) { (void)user; stop_canvas_timer(); detail_back(NULL); }
...
static void open_canvas(void) {
    ...
    gh_detail_section(detail_view, api, "Animated drawing on canvas widget");
    ...
    if (!api->ui_canvas_create || !api->ui_canvas_fill || !api->ui_canvas_draw_rect) {
        ...
        if (sw < 40) sw = 240;
        canvas_obj = api->ui_canvas_create(NULL, sw - 20, 100);
        if (!canvas_obj) api->ui_detail_add_info(detail_view, "Canvas", "Create failed");
    }

    api->ui_detail_add_action(detail_view, "Stop", canvas_stop, NULL);
    api->ui_detail_add_back(detail_view, canvas_stop, NULL);
    create_touch_controls(true);
    if (canvas_obj && api->ui_timer_create) canvas_timer = api->ui_timer_create(canvas_tick_cb, 50, NULL);
}
...
static void open_input(void) {
    ...
    gh_detail_section(detail_view, api, "Press any button to see events");
...
static void open_theme(void) {
    ...
    gh_detail_section(detail_view, api, "Current theme palette");
    gh_detail_printf(detail_view, api, "Background", "#%06lX", (unsigned long)theme.bg);
    gh_detail_printf(detail_view, api, "Surface", "#%06lX", (unsigned long)theme.surface);
    gh_detail_printf(detail_view, api, "Surface Alt", "#%06lX", (unsigned long)theme.surface_alt);
    gh_detail_printf(detail_view, api, "Text", "#%06lX", (unsigned long)theme.text);
    gh_detail_printf(detail_view, api, "Text Muted", "#%06lX", (unsigned long)theme.text_muted);
    gh_detail_printf(detail_view, api, "Accent", "#%06lX", (unsigned long)theme.accent);
    api->ui_detail_add_info(detail_view, "Mode", theme.bright ? "Bright" : "Dark");
...
static void open_unsafe(void) {
    ...
    gh_detail_section(detail_view, api, "Raw LVGL pointers");
    void *scr = api->lv_scr_act ? api->lv_scr_act() : NULL;
    gh_detail_printf(detail_view, api, "lv_scr_act", "%p", scr);
    void *view = api->display_get_current_view ? api->display_get_current_view() : NULL;
    gh_detail_printf(detail_view, api, "Current View", "%p", view);
    void *sym = api->raw_symbol ? api->raw_symbol("lv_scr_act") : NULL;
    gh_detail_printf(detail_view, api, "raw_symbol test", "%p", sym);
...
static void navigate_up(void) {
    if (current_page == PAGE_MENU && main_menu) { GH_VOID(api, ui_options_move_selection, main_menu, -1); return; }
    if (current_page == PAGE_STORAGE && storage_menu) { GH_VOID(api, ui_options_move_selection, storage_menu, -1); return; }
    if (current_page == PAGE_BLE && ble_menu) { GH_VOID(api, ui_options_move_selection, ble_menu, -1); return; }
    if (detail_view) {
        if (api->ui_detail_step_up) api->ui_detail_step_up(detail_view);
        else GH_VOID(api, ui_detail_move_selection, detail_view, -1);
    }
}
...
static void navigate_back(void) {
    if (popup) { popup_close(NULL); return; }
    if (current_page == PAGE_MENU) { exit_app(NULL); return; }
    if (current_page == PAGE_RGB && detail_view) { rgb_stop(NULL); return; }
    if (current_page == PAGE_CANVAS && detail_view) { canvas_stop(NULL); return; }
    if (current_page == PAGE_STORAGE) { if (detail_view) storage_detail_back(NULL); else storage_go_parent(); return; }
    if (current_page == PAGE_BLE) { if (detail_view) detail_back(NULL); else show_menu(); return; }
    if (detail_view) { detail_back(NULL); return; }
    show_menu();
}
...
static bool handle_touch_navigation(const ghostesp_input_event_t *event) {
    ...
    if (swipe == GHOSTESP_INPUT_DOWN) { navigate_down(); return true; }
    if (swipe == GHOSTESP_INPUT_UP) { navigate_up(); return true; }
    if (swipe == GHOSTESP_INPUT_RIGHT) { navigate_back(); return true; }
    return event->pressed || swipe != GHOSTESP_INPUT_NONE;
}
...
static void inspector_input(const ghostesp_input_event_t *event) {
    ...
    /* Input tester page: show raw events */
    if (current_page == PAGE_INPUT && detail_view && api->ui_detail_clear) {
        ...
        gh_detail_section(detail_view, api, "Input Event Received");
        api->ui_detail_add_info(detail_view, "Type", type_name);
        gh_detail_printf(detail_view, api, "Value", "%ld", (long)event->value);
        gh_detail_printf(detail_view, api, "Position", "%ld, %ld", (long)event->x, (long)event->y);
        api->ui_detail_add_info(detail_view, "Pressed", event->pressed ? "YES" : "NO");
        api->ui_detail_add_back(detail_view, detail_back, NULL);
        if (event->type == GHOSTESP_INPUT_BACK) { detail_back(NULL); return; }
        return;
    }
    ...
    ghostesp_options_t active_menu = NULL;
    if (current_page == PAGE_MENU) active_menu = main_menu;
    else if (current_page == PAGE_STORAGE) active_menu = storage_menu;
    else if (current_page == PAGE_BLE) active_menu = ble_menu;

    if (active_menu) {
        if (event->type == GHOSTESP_INPUT_LEFT || event->type == GHOSTESP_INPUT_UP) { GH_VOID(api, ui_options_move_selection, active_menu, -1); return; }
        if (event->type == GHOSTESP_INPUT_RIGHT || event->type == GHOSTESP_INPUT_DOWN) { GH_VOID(api, ui_options_move_selection, active_menu, 1); return; }
        if (event->type == GHOSTESP_INPUT_SELECT) {
            int idx = api->ui_options_get_selected ? api->ui_options_get_selected(active_menu) : -1;
            if (current_page == PAGE_MENU) menu_select((void *)(intptr_t)idx);
            else if (current_page == PAGE_STORAGE) storage_activate_selected();
            else if (current_page == PAGE_BLE) ble_activate_selected();
            return;
        }
    }
    ...
    if (detail_view) {
        ...
        if (event->type == GHOSTESP_INPUT_SELECT) {
            if (api->ui_detail_activate_selected) { api->ui_detail_activate_selected(detail_view); }
            else {
                ...
                if (current_page == PAGE_RGB && selected == count - 2) { rgb_stop(NULL); }
                else if (current_page == PAGE_CANVAS && selected == count - 2) { canvas_stop(NULL); }
                else { detail_back(NULL); }
            }
            return;
        }
    }

    if (event->type == GHOSTESP_INPUT_BACK) navigate_back();
}

static const ghostesp_app_t app = GHOSTESP_APP_DEFINE(
    "device_inspector", "Device Inspector",
    inspector_start, inspector_stop, inspector_input, NULL
);

GHOSTESP_APP_INIT_WITH_API(app, api, "device_inspector", GHOSTESP_API_STRUCT_SIZE_V1)
