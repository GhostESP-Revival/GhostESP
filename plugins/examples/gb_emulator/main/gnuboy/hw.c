...
{
	/* WRAM+VRAM are the hot working set and stay in PSRAM like the rest of
	 * the app's memory (see gb_core.c). */
	hw.rambanks = gnuboy_port_alloc_hot(8 * 4096);
	if (hw.rambanks) memset(hw.rambanks, 0, 8 * 4096);
	hw.vbanks = gnuboy_port_alloc_hot(2 * 8192);
	if (hw.vbanks) memset(hw.vbanks, 0, 2 * 8192);
...
void gb_hw_updatemap(void)
{
	...
	{
		/* GHOSTESP VENDOR PATCH 1: file-based bank loading removed; every
		   declared bank is backed by the host-provided ROM image. Fall back
		   to bank 0 so a malformed cartridge can never index a NULL map. */
		rombank = 0;
	}
