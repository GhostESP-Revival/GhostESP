...
/* GHOSTESP VENDOR PATCH 2: memcmp is not part of the firmware's dynamic
   symbol whitelist for native SD apps; use a small local helper instead. */
static int gnuboy_memcmp_(const void *a, const void *b, size_t len)
{
...
int gnuboy_load_rom(const byte *data, size_t size)
{
	...
	if (gnuboy_memcmp_(cart.name, "SIREN GB2 ", 10) == 0 || gnuboy_memcmp_(cart.name, "DONKEY KONG", 12) == 0)
	{
		MESSAGE_INFO("HACK: Window offset hack enabled (12)\n");
		hw.compat.window_offset = 12;
	}
	else if (gnuboy_memcmp_(cart.name, "RES EVIL GD", 11) == 0 || gnuboy_memcmp_(cart.name, "BIOHAZARDGDB", 12) == 0)
	{
...
{
	/* GHOSTESP VENDOR PATCH 1: ROM banks always point into the host-provided
	   image (loaded from memory), so only the bookkeeping arrays are freed. */
	free(cart.rombanks);
	cart.rombanks = NULL;
