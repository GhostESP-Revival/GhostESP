...
void gb_sound_reset(bool hard)
{
	...
	/* GHOSTESP VENDOR PATCH 5: integer-only rate computation. The upstream
	   double-precision expression imports __adddf3/__divdf3/__fixdfsi/
	   __floatsidf, which are not on the firmware's dynamic symbol whitelist
	   for native SD apps. A zero samplerate disables synthesis entirely
	   (gb_sound_emulate() returns early). */
	snd.rate = host.audio.samplerate > 0
		? (int)(((1u << 21) + (unsigned)host.audio.samplerate / 2) / (unsigned)host.audio.samplerate)
		: 0;
	GB.audio.pos = 0;
	sound_off();
	R_NR52 = 0xF1;
