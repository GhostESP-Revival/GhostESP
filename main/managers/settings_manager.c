...
static const char *NVS_UI_LANGUAGE_KEY = "ui_language";
...
void settings_persist_setting(SettingsType setting) {
    ...
    switch (setting) {
        ...
        case SETTING_LANGUAGE:
            err = nvs_set_u8(nvsHandle, NVS_UI_LANGUAGE_KEY, G_Settings.ui_language);
            key = NVS_UI_LANGUAGE_KEY;
            i18n_set_language((i18n_language_t)G_Settings.ui_language);
            break;
...
#define NVS_SET(call) do { if (err == ESP_OK) { tmp = (call); if (tmp != ESP_OK) { err = tmp; } } } while(0)
    ...
    NVS_SET(nvs_set_u8(nvsHandle, NVS_UI_LANGUAGE_KEY, settings->ui_language));
