/**
 * @file smartrider.c
 * @brief SmartRider parser from Flipper Zero (ported to GhostESP)
 */
...
static bool smartrider_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    const MfClassicData* data = (const MfClassicData*)nfc_device_get_data(device, NfcProtocolMfClassic);
...
const NfcSupportedCardsPlugin smartrider_plugin = {
