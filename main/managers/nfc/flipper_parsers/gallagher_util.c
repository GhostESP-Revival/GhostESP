 ...
 * Source: https://github.com/Next-Flip/Momentum-Firmware/blob/dev/applications/main/nfc/api/gallagher/gallagher_util.c
*/
...
const uint8_t GALLAGHER_CARDAX_ASCII[16] =
...
void gallagher_deobfuscate_and_parse_credential(
    ...
    uint8_t d[8];
    for(int i = 0; i < 8; i++)
        d[i] = GALLAGHER_DECODE_TABLE[cardholder_data_obfuscated[i]];

    credential->region = (d[3] >> 1) & 0x0F;
    credential->facility = ((uint16_t)(d[5] & 0x0F) << 12) +
                           ((uint16_t)d[1] << 4) +
                           (((uint16_t)d[7] >> 4) & 0x0F);
    credential->card = ((uint32_t)d[0] << 16) +
                       ((uint32_t)(d[4] & 0x1F) << 11) +
                       ((uint32_t)d[2] << 3) +
                       (((uint32_t)d[3] >> 5) & 0x07);
    credential->issue = d[7] & 0x0F;
}
