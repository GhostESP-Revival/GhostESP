 ...
 *
 * Originally authored for the Flipper Zero / Momentum-Firmware project.
 * Source: https://github.com/Next-Flip/Momentum-Firmware/blob/dev/applications/main/nfc/plugins/supported_cards/itso.c
 *
 * Adapted for GhostESP: includes changed to flipper_nfc_compat.h, furi_string_set
 * → furi_string_set_str, strint_to_uint32 → strtoul. Parser logic unchanged.
 *
...
static bool itso_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        uint32_t dateStamp = (uint32_t)strtoul(datep, NULL, 16);
        uint32_t unixTimestamp = dateStamp * 24 * 60 * 60 + 852076800U;

        furi_string_set_str(parsed_data, "\e#ITSO Card\n");
...
const NfcSupportedCardsPlugin itso_plugin = {
