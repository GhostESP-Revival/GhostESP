 ...
 * Emily Trau (https://github.com/emilytrau), contributed upstream as
 * https://github.com/flipperdevices/flipperzero-firmware/pull/2326
 * Source: https://github.com/Next-Flip/Momentum-Firmware/blob/dev/applications/main/nfc/plugins/supported_cards/myki.c
 * Card format reference: https://github.com/metrodroid/metrodroid/wiki/Myki
 *
 * Adapted for GhostESP to run against the in-memory MfDesfire tree populated
 * by managers/nfc/desfire.c (see flipper_nfc_compat.h for full DESFire shim
 * attribution). Parser logic is unchanged from the upstream source.
 *
...
static bool myki_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        furi_string_set_str(parsed_data, "\e#myki\nNo.: ");
 ...
 * Must have external linkage: flipper_nfc_compat.c declares it extern and
 * references it from the s_plugins[] registry. */
const NfcSupportedCardsPlugin myki_plugin = {
