...
static bool mizip_read(Nfc* nfc, NfcDevice* device) {
    ...
    do {
        ...
        size_t uid_len = 0;
        const uint8_t* uid_src = mf_classic_get_uid(data, &uid_len);
        if(!uid_src || uid_len < UID_LENGTH) break;
        uint8_t uid[UID_LENGTH];
        memcpy(uid, uid_src, UID_LENGTH);
...
static bool mizip_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        //parse data
        furi_string_cat_printf(parsed_data, "MiZIP Card\n");
...
const NfcSupportedCardsPlugin mizip_plugin = {
