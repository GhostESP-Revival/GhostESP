...
#define REG_OUT_DEFAULT_VAL                                                    \
  ((REG_WR_OC_DEFAULT_VAL << 8) | REG_WR_IO_DEFAULT_VAL)
...
esp_err_t ch422g_new_device(i2c_port_num_t i2c_num, uint32_t i2c_address,
  ...
  ESP_RETURN_ON_FALSE(i2c_num < I2C_NUM_MAX, ESP_ERR_INVALID_ARG, TAG,
                      "Invalid I2C port");
  ESP_RETURN_ON_FALSE(out_dev, ESP_ERR_INVALID_ARG, TAG,
                      "Invalid output device pointer");
...
esp_err_t ch422g_write_output_reg(esp_io_expander_ch422g_t *ch422g,
                                  uint32_t value) {
...
esp_err_t ch422g_read_output_reg(esp_io_expander_ch422g_t *ch422g,
                                 uint32_t *value) {
  *value = ch422g->regs.wr_io | ((uint32_t)ch422g->regs.wr_oc << 8);
  return ESP_OK;
}

esp_err_t ch422g_write_direction_reg(esp_io_expander_ch422g_t *ch422g,
                                     uint32_t value) {
...
esp_err_t ch422g_read_direction_reg(esp_io_expander_ch422g_t *ch422g,
                                    uint32_t *value) {
  *value = (ch422g->regs.wr_set & REG_WR_SET_BIT_IO_OE) ? DIR_OUT_VALUE
                                                        : DIR_IN_VALUE;
  return ESP_OK;
}

esp_err_t ch422g_reset(esp_io_expander_ch422g_t *ch422g) {
  ESP_RETURN_ON_ERROR(ch422g_write_direction_reg(ch422g, REG_DIR_DEFAULT_VAL),
                      TAG, "Failed to reset direction");
  ESP_RETURN_ON_ERROR(ch422g_write_output_reg(ch422g, REG_OUT_DEFAULT_VAL), TAG,
